...
// Node.js para Railway / cloud 24/7
// ============================================
...
function fetchMarketData() {
  return new Promise((resolve, reject) => {
    const body = 'eventId=' + CONFIG.EVENT_ID + '&latestServerStamp=0';
    const req = https.request({
      hostname: 'inv.viagogo.com',
...
function parseSales(html) {
  ...
  const rows = grid.split('<tr class="');
  rows.forEach(row => {
...
function saleKey(s) {
  return s.section + '|' + s.row + '|' + s.seats + '|' + s.qty + '|' + s.priceText;
}
...
async function poll() {
  ...
  try {
    ...
    if (sales.length === 0 && html.length < 500) {
      console.warn('[Monitor] Resposta suspeita — possível sessão expirada. HTML:', html.substring(0, 200));
      await sendTelegram('⚠️ <b>Aviso</b>: Resposta inválida da Viagogo. Sessão pode ter expirado.');
      return;
    }
...
// ---- Arranque ----
console.log('[Monitor] A arrancar...');
poll();
